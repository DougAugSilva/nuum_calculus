...
Matrix fact_LU(Matrix *m, Matrix *b){
    ...
    for(int i = 0; i < n; i++){
        double soma = 0.0;
        for(int j = 0; j < i; j++){
            soma = soma + rab_matrix(&L, i, j) * rab_matrix(&y, j, 0);
        }
        double y_val = (rab_matrix(&bp, i, 0) - soma) / rab_matrix(&L, i, i);
    ...
    for(int i = n - 1; i >= 0; i--){
        double soma = 0.0;
        for(int j = i + 1; j < n; j++){
            soma = soma + rab_matrix(&U, i, j) * rab_matrix(&x, j, 0);
        }
        double x_val = (rab_matrix(&y, i, 0) - soma) / rab_matrix(&U, i, i);
